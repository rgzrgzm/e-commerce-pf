...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (
        estado === "Aprobado" &&
        pedido.dataValues.estado === "Pendiente de pago"
      ) {
        ...
        for (let i = 0; i < compras.length; i++) {
          //Resto el stock
          const productoTalle = await Producto_talle.findOne({
            where: {
          ...
          await productoTalle.update({
            stock:
              productoTalle.dataValues.stock - compras[i].dataValues.cantidad,
          });
      ...
      const montoActualizado = await DatosFactura.update(
        { montoTotal },
        { where: { id: factura.id } }
      );
