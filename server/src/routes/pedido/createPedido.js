  ...
  async (req, res) => {
    try {
      ...
      for (let i = 0; i < productos.length; i++) {
        ...
        if (
          !productoTalle ||
          productoTalle.talles[0].dataValues.producto_talle.dataValues.stock -
            productos[i].cantidad <
            0
        )
          return res
            .status(400)
            .send({ Error: "Hubo un error. Porfavor, inténtelo de vuelta." });
      ...
      for (let i = 0; i < productos.length; i++) {
        const talle = await Talle.findOne({
          where: { talle: productos[i].talle },
        });
        await Compra.create({
