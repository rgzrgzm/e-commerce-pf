...
const { Producto, Talle, Categoria } = require("../../db");
...
router.get("/", async (req, res) => {
  ...
  if (page <= 0 || !page)
    return res
      .status(400)
      .send({
        Error: "Las paginas deben ser números que empiezan desde el 1.",
      });
  ...
  //Si me lo pasan, traigo los productos que tengan un nombre parecido.
    ...
    productsSearch = await Producto.findAndCountAll({
      ...query,
      where: {
        nombre: {
          [Op.iLike]: `%${name}%`,
