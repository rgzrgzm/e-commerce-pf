...
router.put("/", async (req, res) => {
  const { id, nombre, precio, categoria, descripcion, imagen, talle, stock } =
    req.body;
  const productoParaActualizar = { nombre, descripcion, imagen, precio, talle };
  try {
    const prodActualizado = await Producto.update(productoParaActualizar, {
      where: { id: id },
    });
    ...
    if (talle) {
      const tall = await Producto_talle.findAll({
        where: { productoId: prod.dataValues.id },
      });
      tall.map(async (t) => {
        await t.destroy();
      });
      talle.map(async (t, i) => {
        ...
        const updateStock = await Producto_talle.update(
          { stock: stock[i] },
          { where: { productoId: prod.dataValues.id, talleId: tall.id } }
        );
      });
      // talle.map( async (t, i) => {
      //     //Actualizo el stock a el correspondiente
